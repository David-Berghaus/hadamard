...
for i in range(100000000000):
    ...
    if TRAIN_ON_UNIQUE_STATES is True:
        states_batch, unique_indices = np.unique(states_batch, axis=0, return_index=True)
        actions_batch = actions_batch[unique_indices]
        rewards_batch = rewards_batch[unique_indices]
    ...
    if (i%20 == 1): #Write all important info to files every 20 iterations
        with open(data_path+'best_species_pickle'+'.txt', 'wb') as fp:
            pickle.dump(super_actions, fp)
        with open(data_path+'best_species_txt'+'.txt', 'w') as f:
            for item in super_actions:
                f.write(str(item))
                f.write("\n")
        with open(data_path+'best_species_rewards'+'.txt', 'w') as f:
            for item in super_rewards:
                f.write(str(item))
                f.write("\n")
        with open(data_path+'mean_rewards'+'.txt', 'a') as f:
